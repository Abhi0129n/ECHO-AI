...
from tools.calendar.schemas import CalendarEvent, CreateEventRequest, UpdateEventRequest, ReminderRequest
...
class CalendarService:
    def __init__(self, base_dir: str = "."):
        ...
        token_path = os.path.join(self.base_dir, "echo-ai", "config", "calendar_token.json")
        creds_path = os.path.join(self.base_dir, "echo-ai", "config", "credentials.json")
        self.oauth_manager = CalendarOAuthManager(token_path=token_path, creds_path=creds_path)
    ...
    def move_event(self, event_id: str, start_time: str, end_time: str) -> CalendarEvent:
        try:
            existing = self.service.events().get(calendarId='primary', eventId=event_id).execute()
            ...
            raise RuntimeError(f"Error moving event: {str(e)}")
    ...
    def free_slots(self, start_time: str, end_time: str, duration_minutes: int = 30) -> List[Dict[str, str]]:
        ...
        busy = []
        for ev in events:
            ev_start = parse_dt(ev.start_time)
            ev_end = parse_dt(ev.end_time)
            # Only consider events overlapping the query window
            if ev_start < end_dt and ev_end > start_dt:
                clamped_start = max(ev_start, start_dt)
                clamped_end = min(ev_end, end_dt)
